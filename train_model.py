...
def main():
    ...
    print('Testing data size:', Data_TS0.size(0))
    
    datasetTr = dataf.TensorDataset(Data_TR0,  TrainLabel) 
    train_loader = dataf.DataLoader(datasetTr, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
    ...
    for epoch in range(args.epochs):
        ...
        for step, (b_x0,b_y) in enumerate(train_loader):  # gives batch data, normalize x when iterate train_loader
            ...
            torch.save(model.state_dict(), args.saveNet_path + 'net_resnet18.pkl')
            torch.cuda.empty_cache()
            model.train()


    model.load_state_dict(torch.load(args.saveNet_path + 'net_resnet18.pkl'))
    model.eval()
